using System;
using System.Collections.Generic;
using System.Linq;
using System.Net;
using System.Net.Http;
using System.Web.Http;
using gobabygo.BL;
using gobabygo.Models;

namespace gobabygo.Controllers
{
    public class MainController : ApiController
    {
        EnquiryBL objBL = new EnquiryBL();       
        [HttpPost]
        [Route("api/addEnquiry")]
        public Boolean addTables(EnquiryDTO enquiry)
        {
            return objBL.SaveEnquiry(enquiry);
        }
    }
    public class Reqenquiry
    {
        public int id { get; set; }
        public string rFirstName { get; set; }
        public string rLastName { get; set; }
        public string rRelation { get; set; }
        public string rEmail { get; set; }
        public string rPhoneNo { get; set; }
        public string rPhoneType { get; set; }
        public string rAddress { get; set; }
        public string rAddress1 { get; set; }
        public string rCity { get; set; }
        public string rState { get; set; }
        public string rPin { get; set; }
        public string g1FirstName { get; set; }
        public string g1LastName { get; set; }
        public string g1Relation { get; set; }
        public string g1Email { get; set; }
        public string g1PhoneNo { get; set; }
        public string g1PhoneType { get; set; }
        public string g1Address { get; set; }
        public string g1Address1 { get; set; }
        public string g1City { get; set; }
        public string g1State { get; set; }
        public string g1Pin { get; set; }
        public string g2FirstName { get; set; }
        public string g2LastName { get; set; }
        public string g2Relation { get; set; }
        public string g2Email { get; set; }
        public string g2PhoneNo { get; set; }
        public string g2PhoneType { get; set; }
        public string g2Address { get; set; }
        public string g2Address1 { get; set; }
        public string g2City { get; set; }
        public string g2State { get; set; }
        public string g2Pin { get; set; }
        public string hearreason { get; set; }
        public string fname { get; set; }
        public string mname { get; set; }
        public string lname { get; set; }
        public string gender { get; set; }
        public string dob { get; set; }
        public string isAddSame { get; set; }
        public string address1 { get; set; }
        public string address2 { get; set; }
        public string city { get; set; }
        public string cstate { get; set; }
        public string czip { get; set; }
        public string hospitalName { get; set; }
        public string primaryDiagnosis { get; set; }
        public string secondaryDiagnosis { get; set; }
        public string allergies { get; set; }
        public string equipment { get; set; }
        public string measurement1 { get; set; }
        public string measurement2 { get; set; }
        public string cancrawl { get; set; }
        public string cansit { get; set; }
        public string canstand { get; set; }
        public string canwalk { get; set; }
        public string priorexperience { get; set; }
        public string currentSupportings { get; set; }
        public string canpress { get; set; }
        public string picUrl { get; set; }
        public string aboutChild { get; set; }
        public string addinfo1 { get; set; }
        public string addinfo2 { get; set; }
        public string addinfo3 { get; set; }
    }
}
